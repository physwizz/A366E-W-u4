					  ...
					  bool to_lower, bool to_higher, bool force)
{
	...
	if (to_lower) {
		...
		if (per_task_boost(p) == TASK_BOOST_STRICT_MAX &&
			...
			return false;
		if (!force && walt_get_rtg_status(p) && !is_perf_reserve_task(p))
			return false;
		if (!force && !task_fits_max(p, dst_cpu) && !is_perf_reserve_task(p))
			return false;
	} else if (!to_higher) {
	...
	if (is_perf_reserve_task(p) && to_higher && !is_min_possible_cluster_cpu(dst_cpu))
		return false;
				  ...
				  int src_cpu)
{
	...
	if (check_for_higher_capacity(dst_cpu, src_cpu) && is_perf_reserve_task(p))
		return false;
	else if (!check_for_higher_capacity(dst_cpu, src_cpu) && is_perf_reserve_task(p))
		return true;
