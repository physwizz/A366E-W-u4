...
static int proc_perf_reserve_show(struct seq_file *m, void *data)
{
	seq_printf(m, "%u\n", proc_perf_reserve);
	return 0;
}

static int proc_perf_reserve_open(struct inode *inode,
		struct file *file)
{
	return single_open(file, proc_perf_reserve_show, NULL);
}

static ssize_t proc_perf_reserve_write(struct file *file,
...
static const struct proc_ops proc_perf_reserve_op = {
	.proc_open = proc_perf_reserve_open,
	.proc_write = proc_perf_reserve_write,
	.proc_read = seq_read,
