                            ...
                            int (*cmd_noti_reg)(struct notifier_block *nb),int (*cmd_noti_unreg)(struct notifier_block *nb))
{
	...
	pr_info("[AOIS_FACTORY] %s: complete\n", __func__);
}
...
void send_ois_cmd_data(struct adsp_data *data)
{
	...
	do {
		adsp_unicast(cam_ois_cmd_data32, size, MSG_AOIS, 0, MSG_TYPE_SET_REGISTER);
		//pr_info("[AOIS_FACTORY] %s: adsp_unicast complete\n", __func__);

		while (!(data->ready_flag[MSG_TYPE_SET_REGISTER] & 1 << MSG_AOIS) && cnt++ < TIMEOUT_CNT)
			usleep_range(500, 500);

		//pr_info("[AOIS_FACTORY] %s: Ready Flag = %u First\n", __func__, data->ready_flag[MSG_TYPE_SET_REGISTER]);

		data->ready_flag[MSG_TYPE_SET_REGISTER] &= ~(1 << MSG_AOIS);

		//pr_info("[AOIS_FACTORY] %s: Ready Flag = %u Second\n", __func__, data->ready_flag[MSG_TYPE_SET_REGISTER]);

		if (cnt >= TIMEOUT_CNT)
			pr_err("[AOIS_FACTORY] %s: Timeout!!!\n", __func__);
		retry_cnt++;
	} while (cnt >= TIMEOUT_CNT_FOR_OIS && retry_cnt < READ_RETRY_CNT);

	if (retry_cnt >= READ_RETRY_CNT) {
		pr_err("[AOIS_FACTORY] %s: Write Fail!!!\n", __func__);
	} else {
		ret[0] = (int32_t)data->msg_buf[MSG_AOIS][0];
		ret[1] = (int32_t)data->msg_buf[MSG_AOIS][1];
		ret[2] = (int32_t)data->msg_buf[MSG_AOIS][2];
		pr_debug("[AOIS_FACTORY] %s: msg_buf Size = %d, Addr = 0x%x, Val = %d  Count = %d\n",
...
void read_ois_reg_data(struct adsp_data *data,int32_t *reg_data)
{
	...
	do {
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_AOIS, 0, MSG_TYPE_GET_REGISTER);
		//pr_info("[AOIS_FACTORY] %s: adsp_unicast complete\n", __func__);

		cnt = 0;
		while (!(data->ready_flag[MSG_TYPE_GET_REGISTER] & 1 << MSG_AOIS) &&
			cnt++ < TIMEOUT_CNT_FOR_OIS) // 200ms max
			usleep_range(500, 550);

		data->ready_flag[MSG_TYPE_GET_REGISTER] &= ~(1 << MSG_AOIS);
	...
	if (retry_cnt >= READ_RETRY_CNT) {
		pr_err("[AOIS_FACTORY] %s: Read Fail!!!\n", __func__);
	} else {
		pr_debug("[AOIS_FACTORY] %s: RegAddr:[0x%x], AddrType:[%d], DataType(size):[%d], RetVal:[%d], Count:[%d]\n",
								__func__, reg_data[0], reg_data[1], reg_data[2], reg_data[3], cnt);
		for (int i = 0; i <= read_data_sz; ++i) {
			reg_data[i + 3] = (int32_t)data->msg_buf[MSG_AOIS][i];
			pr_debug("[AOIS_FACTORY] %s: reg_data[%d]: %d", __func__, i+3, reg_data[i+3]);
		}
	}
}
void set_factory_mode(struct adsp_data *data,int32_t *reg_data)
{
	...
	pr_info("[AOIS_FACTORY] %s: msg_buf = [%d]\n", __func__, msg_buf[0]);

	do {
		adsp_unicast(msg_buf, sizeof(msg_buf), MSG_AOIS, 0, MSG_TYPE_SET_THRESHOLD);
		//pr_info("[AOIS_FACTORY] %s: adsp_unicast complete\n", __func__);

		cnt = 0;
		while (!(data->ready_flag[MSG_TYPE_SET_THRESHOLD] & 1 << MSG_AOIS) &&
			cnt++ < TIMEOUT_CNT_FOR_OIS) // 200ms max
			usleep_range(500, 550);

		data->ready_flag[MSG_TYPE_SET_THRESHOLD] &= ~(1 << MSG_AOIS);
	...
	if (retry_cnt >= READ_RETRY_CNT)
		pr_err("[AOIS_FACTORY] %s: factory_mode Fail!!!\n", __func__);
	else
		pr_info("[AOIS_FACTORY] %s: factory_mode complete\n", __func__);
}
...
{
	pr_info("[AOIS_FACTORY] : register cam_ois_cmd_init_work notifier\n");
	...
	pr_info("[AOIS_FACTORY] %s: Done!\n", __func__);
}

void cam_ois_reg_read_init_work(struct adsp_data *data)
{
	pr_info("[AOIS_FACTORY] : register cam_ois_reg_read_init_work notifier\n");
...
{
	pr_info("[AOIS_FACTORY] : register cam_ois_factory_mode_init_work notifier\n");
...
int aois_factory_init(void)
{
	adsp_factory_register(MSG_AOIS, ois_gyro_attrs);
	pr_info("[AOIS_FACTORY] %s\n", __func__);
	return 0;
}

void aois_factory_exit(void)
{
	adsp_factory_unregister(MSG_AOIS);
	pr_info("[AOIS_FACTORY] %s\n", __func__);
}