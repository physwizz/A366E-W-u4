			    ...
			    bool rtnl_held)
{
	...
	if (!reinit) {
		...
		qdf_atomic_init(&adapter->deflink->session.ap.is_ap_suspend);
	}
				...
				bool check_for_concurrency)
{
	...
	ap_ctx->dfs_cac_block_tx = true;
	qdf_atomic_set_bit(SOFTAP_INIT_DONE,
			   link_info->link_flags);
	...
	qdf_atomic_set_bit(SOFTAP_BSS_STARTED,
			   link_info->link_flags);
...
error:
	...
	qdf_atomic_clear_bit(SOFTAP_INIT_DONE,
			     link_info->link_flags);
	qdf_atomic_set(&ap_ctx->acs_in_progress, 0);
	wlansap_reset_sap_config_add_ie(config, eUPDATE_IE_ALL);
