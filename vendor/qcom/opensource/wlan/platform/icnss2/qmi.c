...
static void cnss_add_ss_naming_rule(struct icnss_priv *priv,
		char *filename)
{
	char ant[3];
	const char *postfix = "";
	...
	icnss_pr_err("ant_from_macloader %d\n", ant_from_macloader);
	/*
	   ant_from_macloader
	...
	of_property_read_string(priv->pdev->dev.of_node,
			"qcom,bdf-postfix-name", &postfix);
	len = strlen(postfix);

	if (len == 0)
		icnss_pr_dbg("Failed to get qcom,bdf-postfix-name\n");
	else if (len + strlen(filename) > MAX_FIRMWARE_NAME_LEN )
		icnss_pr_err("Over MAX_FIRMWARE_NAME_LEN\n");
	else
	...
	case ICNSS_BDF_ELF:
		if (priv->board_id == 0xFF)
			...
			snprintf(filename_tmp, filename_len,
				 ...
				 priv->board_id & 0xFF);
#ifdef CONFIG_SEC_SS_CNSS_FEATURE_SYSFS
		cnss_add_ss_naming_rule(priv, filename_tmp);
#endif /* CONFIG_SEC_SS_CNSS_FEATURE_SYSFS */
		break;
	case ICNSS_BDF_BIN:
		if (priv->board_id == 0xFF)
