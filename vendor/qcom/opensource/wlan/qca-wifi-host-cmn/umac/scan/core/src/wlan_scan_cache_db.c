...
QDF_STATUS __scm_handle_bcn_probe(struct scan_bcn_probe_event *bcn)
{
	...
	QDF_STATUS status = QDF_STATUS_SUCCESS;
	uint32_t list_count, i, sap_count;
	qdf_list_node_t *next_node = NULL;
	struct scan_cache_node *scan_node;
	...
	for (i = 0; i < list_count; i++) {
		...
		/*
		 * On non-DBS Hw if skip_6g_and_indoor_freq = 1, STA should
		 * avoid connecting on 6 GHz channel when SAP is active.
		 * But if SAP is active on 6Ghz & scan is issued, then when
		 * firmware rests on home channel, the home 6Ghz channel
		 * beacons are forwarded to userspace, leading to the userspace
		 * selecting 6 GHz channel for connection. So drop those beacons
		 * received on 6 GHz/indoor frequencies when SAP is active &
		 * skip_6g_and_indoor_freq is 1.
		 */
		...
		if ((wlan_reg_is_6ghz_chan_freq(scan_entry->channel.chan_freq) ||
		     ...
		     scan_obj->scan_def.skip_6g_and_indoor_freq && sap_count) {
			scm_info_rl(QDF_MAC_ADDR_FMT ": Drop frame(%d) received on 6GHz(%d) when sap is present",
				    QDF_MAC_ADDR_REF(scan_entry->bssid.bytes),
				    bcn->frm_type,
				    scan_entry->channel.chan_freq);
			util_scan_free_cache_entry(scan_entry);
