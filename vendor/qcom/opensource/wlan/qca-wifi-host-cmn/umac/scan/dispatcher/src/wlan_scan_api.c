...
wlan_scan_update_hint_bssid(struct wlan_objmgr_psoc *psoc,
			    struct scan_start_request *req,
			    qdf_freq_t ll_sap_freq, bool skip_6Ghz_indoor_freq)
{
	...
	for (i = 0; i < req->scan_req.num_hint_bssid; i++) {
		...
		if (ll_sap_freq &&
		    policy_mgr_2_freq_always_on_same_mac(psoc, ll_sap_freq,
							 freq))
			continue;

		if (skip_6Ghz_indoor_freq &&
		    (wlan_reg_is_6ghz_chan_freq(freq) ||
		     wlan_reg_is_freq_indoor(wlan_vdev_get_pdev(req->vdev),
					     freq)))
			continue;
 ...
 */
	...
	wlan_scan_update_hint_bssid(psoc, req, ll_sap_freq, false);
	wlan_scan_update_hint_s_ssid(psoc, req, ll_sap_freq, false);
	/*
