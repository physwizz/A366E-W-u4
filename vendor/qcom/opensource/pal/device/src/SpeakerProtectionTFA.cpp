...
#define PAL_SP_TFA_VERSION "Goodix_TFA9878_SB55_A36_20241119_1"
...
#define DUMMY_CAL_VALUE 6200 /* top spk. */
#define DUMMY_CAL_VALUE_R 6400 /* bottom spk. */
#define MAX_RE25C_VALUE 6553600 // 100 ohm : 100 * 65536

#define TFA_PRIMARY_DEVICE 0
#define TFA_SECONDARY_DEVICE 1
#define DUMMY_CAL_FILE "/data/vendor/audio/dummy_tfa.cal"
#define DUMMY_CAL_FILE_R "/data/vendor/audio/dummy_tfa_r.cal"
...
int32_t SpeakerProtectionTFA::tfa_dsp_get_param(uint8_t *buffer, int buf_len, uint32_t param_id)
{
    const char *controlName = "getParam";
    apm_module_param_data_t *header = NULL;
    struct mixer_ctl *ctrl = NULL;
    ...
    PAL_DBG(LOG_TAG, "buffer len %d", buf_len);
    status = tfa_dsp_get_miid(controlName, &ctrl);

    if (ctrl != NULL && buffer != NULL && buf_len > 0 && status == TFA_NO_ERR) {
        char *payload;
        int pkg_size = PAL_ALIGN_8BYTE(buf_len + sizeof(apm_module_param_data_t));
        ...
        if (payload) {
            ...
            if (status == TFA_NO_ERR) {
                memcpy(buffer, payload + sizeof(apm_module_param_data_t), buf_len);
                if (buf_len >= 4)
                    PAL_DBG(LOG_TAG, "read buffer : 0x%x, 0x%x, 0x%x, 0x%x", buffer[0], buffer[1], buffer[2], buffer[3]);
            }
            free(payload);
                        ...
                        std::shared_ptr<ResourceManager> Rm):Device(device, Rm)
{
    ...
    cal_val_p = get_dummy_cal(TFA_PRIMARY_DEVICE);
    cal_val_s = get_dummy_cal(TFA_SECONDARY_DEVICE);
...
int32_t SpeakerProtectionTFA::setParameter(uint32_t param_id, void *param)
{
    ...
    switch (param_id) {
        ...
        case PAL_PARAM_ID_SPEAKER_AMP_BIGDATA_START:
            ...
            if (active_spk_dev_count > 0 && tfadsp_state == TFADSP_STATE_IDLE) {
        ...
        case PAL_PARAM_ID_SPEAKER_AMP_TEMPERATURE_RCV:
            ...
            if (debug_mode == true) {
                if (tfadsp_state >= TFADSP_STATE_ACTIVE) {
                    ...
                    int32_t state = 0;
                    int32_t ret = tfa_dsp_get_param(cmd_buf, 4, PARAM_ID_TFADSP_GET_STATE);
                    if (ret == TFA_NO_ERR) {
        ...
        case PAL_PARAM_ID_SPEAKER_AMP_TEMPERATURE_SPK:
            ...
            if (debug_mode == true) {
                if (tfadsp_state >= TFADSP_STATE_ACTIVE) {
                    uint8_t cmd_buf[12] = {0, };
                    int32_t state = 0;
                    int32_t ret = tfa_dsp_get_param(cmd_buf, 4, PARAM_ID_TFADSP_GET_PROCESS_STATE);
                    if (ret == TFA_NO_ERR) {
                        state = *((int32_t *)cmd_buf);
                        amp_ssrm.temperature = state;
                        *param = (void *)&amp_ssrm;
...
int32_t SpeakerProtectionTFA::activate_tfadsp()
{
    ...
    for (int i = 0; i < TFADSP_ACTIVATION_POLLING_MAX_COUNT; i++) {
        if (activate_tfadsp_thread_running == false)
            break;
        PAL_DBG(LOG_TAG, "tfa_dsp_get_param read_state polling %d", i+1);
        memset(&cmd_buf[0], 0, 12); // reset buffer
        ret = tfa_dsp_get_param(cmd_buf, 4, PARAM_ID_TFADSP_GET_STATE);
        state = *((uint32_t *)cmd_buf);
        if (ret == TFA_NO_ERR && state != TFADSP_STATE_IDLE) {
            PAL_DBG(LOG_TAG, "read_state success, tfadsp_state %d", state);
            break;
        }
        if (activate_tfadsp_thread_running == false)
            break;
    ...
    if (cal_val_p == 0)
        cal_p = (DUMMY_CAL_VALUE*65536)/1000;
    else
        cal_p = (cal_val_p*65536)/1000;
    if (cal_val_s == 0)
        cal_s = (DUMMY_CAL_VALUE_R*65536)/1000;
    else
    ...
    if (ret == TFA_NO_ERR) {
        ...
        if (ret == TFA_NO_ERR) {
            state = *((uint32_t *)cmd_buf);
            PAL_DBG(LOG_TAG, "tfa_dsp_get_param, tfadsp state %d", state);
            usleep(TFADSP_ACTIVATION_POLLING_INTERVAL*20); // 40ms
            ret = tfa_dsp_get_param(cmd_buf, 4, PARAM_ID_TFADSP_GET_PROCESS_STATE);
            if (ret == TFA_NO_ERR) {
                state = *((uint32_t *)cmd_buf);
                PAL_DBG(LOG_TAG, "tfa_dsp_get_param, tfadsp_process state %d", state);
            }
        }
    }
#endif
...
int32_t SpeakerProtectionTFA::get_rdc(uint32_t *mOhm_P, uint32_t *mOhm_S)
{
    ...
    *mOhm_S = 0;
    ...
    if (*mOhm_P == 0)
        *mOhm_P = get_dummy_cal(TFA_PRIMARY_DEVICE);
    ...
    if (*mOhm_S == 0)
        *mOhm_S = get_dummy_cal(TFA_SECONDARY_DEVICE);
...
void SpeakerProtectionTFA::get_spkt_data()
{
    ...
    if (cal_status == TFA_CAL_IDLE && (power_state == 0 || power_state_r == 0)) {
        ...
        memcpy(&read_buf[0], (uint8_t *)&cmd, 4);
        ipc_mutex.lock();
        ret = tfa_dsp_msg_write(PARAM_ID_TFADSP_GET_SSRM, (const uint8_t *)read_buf, 4);
        if (ret == TFA_NO_ERR) {
            memset(read_buf, 0, sizeof(read_buf));
            ret = tfa_dsp_get_param(read_buf, sizeof(read_buf), PARAM_ID_TFADSP_GET_SSRM);
        }
        ipc_mutex.unlock();
        if (ret == TFA_NO_ERR) {
            res_buf = (uint32_t *)read_buf;
...
void SpeakerProtectionTFA::Livedata_Logging_Thread()
{
    ...
    memset(memtrack_buf, 0, TFADSP_LIVEDATA_MEMTRACK_MAX_SIZE);
    read_size = fread(memtrack_buf, sizeof(uint8_t), TFADSP_LIVEDATA_MEMTRACK_MAX_SIZE-8, fp_set_memtrack);
    ...
    pCmd = (uint32_t *)memtrack_buf;
    PAL_DBG(LOG_TAG, "tfa_dsp_msg_write 0x%x, 0x%x", pCmd[0], pCmd[1]);
    ret = tfa_dsp_msg_write(PARAM_ID_TFADSP_SET_COMMAND, (const uint8_t *)memtrack_buf, read_size);
    if (ret == TFA_NO_ERR) {
        ret = tfa_dsp_msg_read(memtrack_buf, 16);
    }
    if (ret != TFA_NO_ERR)
        goto exit;

    PAL_DBG(LOG_TAG, "tfa_dsp_msg_read 0x%x, 0x%x", pCmd[0], pCmd[1]);
    fwrite(memtrack_buf, sizeof(uint8_t), 16, fp_memtrack_data);

    while(true) {
        if (livedata_logging_thread_running == false)
            break;

        memcpy(&memtrack_buf[0], (uint8_t *)&get_memtrack, 4);
        ret = tfa_dsp_msg_write(PARAM_ID_TFADSP_SET_COMMAND, (const uint8_t *)memtrack_buf, 4);
        if (ret != TFA_NO_ERR)
            break;
        ret = tfa_dsp_msg_read(memtrack_buf, read_size+8);
        if (ret != TFA_NO_ERR)
            break;
        PAL_DBG(LOG_TAG, "livedata read size %d", read_size+8);
        fwrite(memtrack_buf, sizeof(uint8_t), read_size+8, fp_memtrack_data);
...
uint32_t SpeakerProtectionTFA::get_dummy_cal(uint32_t dev_idx)
{
    uint32_t dummy_cal = 0;
    FILE *fd_cal = NULL;

    if (dev_idx == TFA_PRIMARY_DEVICE) {
        dummy_cal = DUMMY_CAL_VALUE;
        fd_cal = fopen(DUMMY_CAL_FILE, "r");
    } else if (dev_idx == TFA_SECONDARY_DEVICE) {
        dummy_cal = DUMMY_CAL_VALUE_R;
        fd_cal = fopen(DUMMY_CAL_FILE_R, "r");
    }
    if (fd_cal) {
        char str_cal[8] = {0, };
        if (fread(str_cal, 1, sizeof(str_cal), fd_cal) > 0) {
            sscanf(str_cal, "%d", &dummy_cal);
        }
        fclose(fd_cal);
    }
    PAL_DBG(LOG_TAG, "dev_idx %d, dummy_cal %d", dev_idx, dummy_cal);
    return dummy_cal;
}
