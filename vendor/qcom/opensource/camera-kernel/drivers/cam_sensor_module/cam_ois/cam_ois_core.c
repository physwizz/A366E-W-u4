...
#define QTIMER			    (0x566) 						// QIS
uint32_t cam_ois_reg_data1[MAX_CMD_DATA_SIZE];
int32_t cam_ois_reg_read_data1[MAX_CMD_DATA_SIZE+4];
#endif
...
#endif
{
...
#if defined(CONFIG_SAMSUNG_OIS_MCU_STM32)		
		if (o_ctrl->is_power_up) return 0;
	...
	o_ctrl->is_power_up = true;
#if defined(CONFIG_SEC_CAM_OIS_ALGO)
	cam_ois_AOIS_Enable(AOIS_ENABLE);
#endif
#endif
	return rc;
cci_failure:
...
#endif
{
	...
	if (!o_ctrl->is_power_up)
		...
		cam_ois_AOIS_Enable(AOIS_DISABLE);
#endif		
//		return 0;
//	if (!o_ctrl->is_power_up)
//		return 0;
	...
	list_for_each_entry(i2c_list,
		...
		if (i2c_list->op_code ==  CAM_SENSOR_I2C_WRITE_RANDOM) {
			...
			for(i=0; i<size; i++) {
				...
				cam_ois_reg_write_notifier_call_chain(0, cam_ois_reg_data1);
				CAM_INFO(CAM_OIS,"call cam_ois_reg_write_notifier_call_chain() addr = 0x%x, data = 0x%x", cam_ois_reg_data1[1],cam_ois_reg_data1[4]);
			}
#else				
			...
			if (o_ctrl->io_master_info.master_type == CCI_MASTER) {
				...
				cam_ois_reg_data1[0] = size;
				cam_ois_reg_data1[1] = i2c_list->i2c_settings.reg_setting[i].reg_addr;
				cam_ois_reg_data1[2] = i2c_list->i2c_settings.addr_type;
				cam_ois_reg_data1[3] = size;

				
				for(i=0; i<size; i++) {
					cam_ois_reg_data1[4+i] = i2c_list->i2c_settings.reg_setting[i].reg_data;	
				}
				
				cam_ois_reg_write_notifier_call_chain(0, cam_ois_reg_data1);
				CAM_INFO(CAM_OIS,"call cam_ois_reg_write_notifier_call_chain() addr = 0x%x, length = 0x%x, data[0] = 0x%x, data[1] = 0x%x, data[2] = 0x%x, data[3] = 0x%x", cam_ois_reg_data1[1],size, cam_ois_reg_data1[4], cam_ois_reg_data1[5], cam_ois_reg_data1[6], cam_ois_reg_data1[7]);
	...
	uint32_t *cmd_buf, size_t len)
{
	...
	if (!o_ctrl)
	{
		CAM_ERR(CAM_OIS, "Invalid o_ctrl");
	}
	
	if (!cmd_buf)
	{
		CAM_ERR(CAM_OIS, "Invalid cmd_buf");
	}
	
	if (len < sizeof(struct cam_cmd_ois_info))
	{
		CAM_ERR(CAM_OIS, "Invalid len");
	}
	
	if (!o_ctrl || !cmd_buf || len < sizeof(struct cam_cmd_ois_info)) {
		CAM_ERR(CAM_OIS, "Invalid Args");
		CAM_ERR(CAM_OIS,"len :%d",len);
		CAM_ERR(CAM_OIS,"Sizeof :%d",sizeof(struct cam_cmd_ois_info));
		return -EINVAL;
	}
...
static int cam_ois_fw_download(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_INFO(CAM_OIS, "FW prog size:%d.", total_bytes);
	...
	CAM_INFO(CAM_OIS, "FW coeff size:%d", total_bytes);
...
static int cam_ois_pkt_parse(struct cam_ois_ctrl_t *o_ctrl, void *arg)
{
	...
	CAM_ERR(CAM_OIS, "CAM_OIS_PACKET_OPCODE_INIT");
		...
		CAM_INFO(CAM_OIS, "num_cmd_buf %d",
		...
		if (rc < 0) {
			...
			CAM_INFO(CAM_OIS, "apply Init settings success");
		}
		...
		if (o_ctrl->is_ois_calib) {
			...
			if (rc) {
				...
				CAM_INFO(CAM_OIS, "apply calib data settings success");
			}
		}
	...
	case CAM_OIS_PACKET_OPCODE_READ: {
		...
		list_for_each_entry(i2c_list, &(i2c_read_settings.list_head), list) {
			...
			size = i2c_list->i2c_settings.size;
			data_type = i2c_list->i2c_settings.data_type;
			read_buff = i2c_list->i2c_settings.read_buff;

			cam_ois_reg_read_data1[0] = (int32_t)i2c_list->i2c_settings.reg_setting[0].reg_addr;
			cam_ois_reg_read_data1[1] = i2c_list->i2c_settings.addr_type;
			cam_ois_reg_read_data1[2] = 58;
			cam_ois_reg_read_data1[3] = -1;
			cam_ois_reg_read_notifier_call_chain(0, &cam_ois_reg_read_data1);
	
			CAM_INFO(CAM_OIS,"[QIS][Read] cam_ois_reg_read_notifier_call_chain() addr = 0x%x, data = 0x%x", cam_ois_reg_read_data1[0],cam_ois_reg_read_data1[3]);		
			for (int k = 0; k < 62; k++) {
					read_buff[k] = (uint8_t)cam_ois_reg_read_data1[4+k];
            }

			

			cam_ois_reg_data1[0] = 1;
			cam_ois_reg_data1[1] = QTIMER;
			cam_ois_reg_data1[2] = i2c_list->i2c_settings.addr_type;
			cam_ois_reg_data1[3] = i2c_list->i2c_settings.data_type;
			cam_ois_reg_data1[4] = 1;	
			cam_ois_reg_write_notifier_call_chain(0, cam_ois_reg_data1);
#endif
		}

#if defined(CONFIG_SEC_CAM_OIS_ALGOO)
		...
		if ((ois_ts < OIS_MCU_TS_OFFSET_MIN_US) || (ois_ts >= OIS_MCU_TS_OFFSET_MAX_US) || (op_time > OIS_RW_OP_TIME_CHECK_MAX_NS) || (valid_num == 0) || (valid_num >= OIS_READ_BUFFER_VALID_SAMPLE_NUM_MAX)) {
