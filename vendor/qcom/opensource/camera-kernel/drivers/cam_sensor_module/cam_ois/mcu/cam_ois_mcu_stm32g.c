...
#define MAX_CMD_DATA_SIZE   (62)
static uint32_t cam_ois_reg_data[MAX_CMD_DATA_SIZE];
static int32_t cam_ois_reg_read_data[MAX_CMD_DATA_SIZE+4];
static int32_t cam_ois_factory_mode_data[4];
...
int target_validation(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_INFO(CAM_OIS, "1. Connection OK");

	ret = sysboot_i2c_info(o_ctrl);
	if (ret < 0)
	{
		CAM_INFO(CAM_OIS, "Error: Failed to collect the target info (%d)", ret);
		goto validation_fail;
	}
...
int target_empty_check_status(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	CAM_INFO(CAM_OIS, "[INF] Flash Word: 0x%08X", value);
...
int target_read_hwver(struct cam_ois_ctrl_t *o_ctrl){
	...
	for(i = 0; i<4 ; i++){
		...
		if(ret < 0){
			...
			CAM_INFO(CAM_OIS,"read fwver addr 0x%x dst 0x%x", address, dst);
		}
	}
	return ret ;
}
...
int cam_ois_AOIS_Enable_notifier_call_chain(unsigned long val, void *v)
{
    return blocking_notifier_call_chain(&cam_ois_factory_mode_data_nb_head, val, v);
}
int cam_ois_AOIS_Enable_notifier_register(struct notifier_block *nb)
{
    return blocking_notifier_chain_register(&cam_ois_factory_mode_data_nb_head, nb);
}
EXPORT_SYMBOL(cam_ois_AOIS_Enable_notifier_register);

int cam_ois_AOIS_Enable_notifier_unregister(struct notifier_block *nb)
{
    return blocking_notifier_chain_unregister(&cam_ois_factory_mode_data_nb_head, nb);
}
EXPORT_SYMBOL(cam_ois_AOIS_Enable_notifier_unregister);

int cam_ois_reg_read_notifier_call_chain(unsigned long val, void*v)
{
    return blocking_notifier_call_chain(&cam_ois_reg_read_data_nb_head, val, v);    
}
...
int cam_ois_reg_write_notifier_call_chain(unsigned long val, void *v)
{
    return blocking_notifier_call_chain(&cam_ois_write_reg_data_nb_head, val, v);
}

int cam_ois_reg_write_notifier_register(struct notifier_block *nb)
{
    return blocking_notifier_chain_register(&cam_ois_write_reg_data_nb_head, nb);
}
EXPORT_SYMBOL(cam_ois_reg_write_notifier_register);

int cam_ois_reg_write_notifier_unregister(struct notifier_block *nb)
{
    return blocking_notifier_chain_unregister(&cam_ois_write_reg_data_nb_head, nb); 
}
EXPORT_SYMBOL(cam_ois_reg_write_notifier_unregister);

int cam_ois_AOIS_Enable(int op)
{
    int rc = -1;
    cam_ois_factory_mode_data[0] = op;
    cam_ois_factory_mode_data[1] = -1;
    cam_ois_factory_mode_data[2] = -1;
    cam_ois_factory_mode_data[3] = -1;
			
    rc = cam_ois_AOIS_Enable_notifier_call_chain(0, &cam_ois_factory_mode_data);
    
    if(op == AOIS_ENABLE)
        CAM_INFO(CAM_OIS, "AOIS_Enable!");
    else
        CAM_INFO(CAM_OIS, "AOIS_Disable");
    ...
    enum camera_sensor_i2c_type data_type)
{
	...
	int i=0;
    rc = -1;
    cam_ois_reg_read_data[0] = addr;
    cam_ois_reg_read_data[1] = addr_type;
    cam_ois_reg_read_data[2] = data_type;
    cam_ois_reg_read_data[3] = -1;

    rc = cam_ois_reg_read_notifier_call_chain(0, &cam_ois_reg_read_data);
    if (cam_ois_reg_read_data[3] == -1) {
        CAM_ERR(CAM_OIS, "ois i2c byte read failed addr : 0x%x data : 0x%x", cam_ois_reg_read_data[0], cam_ois_reg_read_data[3]);
        cam_ois_reg_read_data[3] = 0;
    }

	if (data_type == CAMERA_SENSOR_I2C_TYPE_BYTE)
		*data = cam_ois_reg_read_data[4];
	else if (data_type == CAMERA_SENSOR_I2C_TYPE_WORD)
		*data = cam_ois_reg_read_data[4] << 8 | cam_ois_reg_read_data[5];
	else if (data_type == CAMERA_SENSOR_I2C_TYPE_3B)
		*data = cam_ois_reg_read_data[4] << 16 | cam_ois_reg_read_data[5] << 8 | cam_ois_reg_read_data[6];
	else if (data_type == CAMERA_SENSOR_I2C_TYPE_DWORD)
		*data = cam_ois_reg_read_data[4] << 24 | cam_ois_reg_read_data[5] << 16 |cam_ois_reg_read_data[6] << 8 | cam_ois_reg_read_data[7];
	else
	{
		if (data_type < MAX_CMD_DATA_SIZE+1)
		{
			for (i=0; i<data_type; i++) data[i] = cam_ois_reg_read_data[4+i];
		}
    }
#else
    uint32_t temp;
    ...
    *data = temp;
#endif
    CAM_INFO(CAM_OIS, "addr = 0x%x data: 0x%x", addr, *data);
    return rc;
}
...
#if defined(CONFIG_SEC_CAM_OIS_ALGO)
    cam_ois_reg_data[0] = 1;
    cam_ois_reg_data[1] = addr;
    cam_ois_reg_data[2] = addr_type;
    cam_ois_reg_data[3] = data_type;
    cam_ois_reg_data[4] = data;
    cam_ois_reg_write_notifier_call_chain(0, cam_ois_reg_data);
#else
...
#if defined(CONFIG_SEC_CAM_OIS_ALGO)
        if(i < MAX_CMD_DATA_SIZE - 4)
        {
            cam_ois_reg_data[i + 4] = data[i];
        }
        else
        {
            pr_info("[SPI_TEST] data_size is more than 28\n");
        }
        //pri_info("%s [SPI_TEST] data_size = %u, reg_addr = %u, data = %u\n", __func__, data_size, addr, data[i]);
#endif
    }
#if defined(CONFIG_SEC_CAM_OIS_ALGO)
    cam_ois_reg_data[0] = data_size;
    cam_ois_reg_data[1] = addr;
    cam_ois_reg_data[2] = addr_type;
    cam_ois_reg_data[3] = data_type;
    cam_ois_reg_write_notifier_call_chain(0, cam_ois_reg_data);
	...
	uint8_t ucSize)
{
	...
	for (i = 0; i < ucSize; i++) {
		ret |= cam_ois_i2c_read(o_ctrl, 0x0106 + i, &RcvData,
			CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
		CAM_INFO(CAM_OIS, "RcvData[0x%x] %d", 0x0106 + i, RcvData);
		*(pBuf + i) = (RcvData & 0xFF);
	}
...
int cam_ois_init(struct cam_ois_ctrl_t *o_ctrl)
{
...
#endif
						default:
							CAM_INFO(CAM_UTIL, "[NON][OIS][%d] Unsupport\n", hw_cam_position);
							break;
						}
					}
				}
	...
	CAM_INFO(CAM_OIS, "ois read hall x center %d", o_ctrl->x_center);
	...
	CAM_INFO(CAM_OIS, "ois read hall y center %d", o_ctrl->y_center);
...
{
    int i = 0;
    uint8_t data[OIS_VER_SIZE + 1] = {'1','3','S','B','N','M','2','T',};
...
int32_t cam_ois_read_module_ver(struct cam_ois_ctrl_t *o_ctrl)
{
    ...
    if (rc < 0)
        CAM_ERR(CAM_OIS, "vdr char is not alnum type.");
        return -2;
    }
    o_ctrl->module_ver[0] = data[3]; // core version
    o_ctrl->module_ver[1] = data[2];
...
int cam_ois_shift_calibration(struct cam_ois_ctrl_t *o_ctrl, uint16_t af_position, uint16_t subdev_id)
{
	...
	//CAM_INFO(CAM_OIS, "cam_ois_shift_calibration %d, subdev: %d", af_position, subdev_id);
	...
	bool is_force_update)
{
	...
	while (len > 0)
	{
	  ...
	  /* write the unit */
	  CAM_INFO(CAM_OIS, "[OIS_FW_DBG] write wbytes=%d  left len=%d", wbytes, len);
	  for(i = 0; i<wbytes; i++ ){
	      sendData[i] = buffer[i];
	  }
	...
	long *raw_data_x, long *raw_data_y, long *raw_data_z)
{
	...
	CAM_INFO(CAM_OIS, "XGZERO 0x%x", xgzero_val);
	...
	CAM_INFO(CAM_OIS, "YGZERO 0x%x", ygzero_val);
	...
	*raw_data_y = ygzero_val * 1000 / scale_factor;
	...
	CAM_INFO(CAM_OIS, "ZGZERO 0x%x", zgzero_val);
	...
	uint8_t *buf, uint32_t buf_size, long *raw_data_x, long *raw_data_y, long *raw_data_z)
{
	...
	CAM_INFO(CAM_OIS, "cam_ois_parsing_raw_data E");
	if (!o_ctrl)
		return 0;
...
uint32_t cam_ois_self_test(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	/* OIS Status Check */
	CAM_INFO(CAM_OIS, "GyroSensorSelfTest E");
	if (!o_ctrl)
		return -1;
	...
	CAM_INFO(CAM_OIS, "GyroSensorSelfTest X");
	return RcvData;
}
...
{
	int i=0, rc = 0;
	int32_t hw_ver_addr = 0x00F8;
	int32_t vdr_info_addr = 0x007C;
	uint32_t val=0;
	char fw_ver[9] = {0,};
	char fw_ver_write[9] = {0,};
	...
	for (i=0;i<4;i++)
	{
		cam_ois_i2c_read(o_ctrl, hw_ver_addr + i, &val,
			CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
			
		if (rc< 0) 	CAM_ERR(CAM_OIS, "get ois status register value failed, i2c fail");
	...
	for (i=0;i<4;i++)
	{
		cam_ois_i2c_read(o_ctrl, vdr_info_addr + i, &val,
			CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
	...
	fw_ver_write[0] = fw_ver[3]; // core version
	fw_ver_write[1] = fw_ver[2];
	fw_ver_write[2] = fw_ver[1]; // MCU infor
	fw_ver_write[3] = fw_ver[0]; // Gyro
	fw_ver_write[4] = fw_ver[4]; // FW release year
	fw_ver_write[5] = fw_ver[5]; // FW release month
	fw_ver_write[6] = fw_ver[6]; // FW release count
	fw_ver_write[7] = fw_ver[7]; // Dev or Rel
	fw_ver[8] = '\0';
	fw_ver_write[8] = '\0';
		
	CAM_INFO(CAM_OIS, "fw_ver:[%s] fw_ver_write:[%s]!", fw_ver, fw_ver_write);
	
	snprintf(ois_fw_full, 40, "%s  \n", fw_ver_write);
...
#if 0
            rc = cam_ois_rumba_fw_update(o_ctrl, is_force_update);
#else
...
int cam_ois_fixed_aperture(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	val = o_ctrl->x_center;
	CAM_INFO(CAM_OIS, "Write X center %d", val);
	data[0] = val & 0xFF;
	data[1] = (val >> 8) & 0xFF;
	...
	val = o_ctrl->y_center;
	CAM_INFO(CAM_OIS, "Write Y center %d", val);
	data[0] = val & 0xFF;
	data[1] = (val >> 8) & 0xFF;
	...
	for (i = 0; i < CUR_MODULE_NUM; i++) {
		if (*(cal_mark[i]) == 0xBB) {
			...
			for (j = 0; j < OIS_XYGG_SIZE ; j++)
				CAM_INFO(CAM_OIS, "[0x%x] 0x%x", XGG_ADDR[i] + j, *(xygg[i] + j));
		}
	}
...
int cam_ois_write_dual_cal(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	for (i = 0; i < CUR_MODULE_NUM; i++) {
		if (*(cal_mark[i]) == 0xBB) {
			...
			for (j = 0; j < OIS_CENTER_SHIFT_SIZE ; j++)
				CAM_INFO(CAM_OIS, "[0x%x] 0x%x", XCOFFSET_ADDR[i] + j, *(center_shift[i] + j));
		}
	}
#endif
	...
	if (0 != o_ctrl->efs_cal) {
		...
		for (j = 0; j < OIS_CENTER_SHIFT_SIZE ; j++)
			CAM_INFO(CAM_OIS, "[0x%x] 0x%x", XCOFFSET_ADDR[efs_index] + j, *(efs_center_shift + j));
	}
#endif
...
int cam_ois_write_gyro_orientation(struct cam_ois_ctrl_t *o_ctrl)
{
	...
	for (i = 0; i < CUR_MODULE_NUM; i++) {
		...
		CAM_INFO(CAM_OIS, "M%u Tx Pole %u, Ty Pole %u", (i + 1), sendData[0], sendData[1]);
	...
	CAM_INFO(CAM_OIS, "GyroOrientation 0x%x", o_ctrl->gyro_orientation);
    ...
    uint32_t* targetPosition, uint32_t* hallPosition)
{
	...
	//OIS Centering
	int rc_set = cam_ois_set_ois_mode(o_ctrl, 0x05);
	if (rc_set < 0) {
		CAM_ERR(CAM_OIS, "ois set fixed mode failed %d", rc);
		return rc_set;
	}
	...
	uint8_t* pBuf, uint8_t ucSize)
{
	...
	for (i = 0; i < ucSize; i++) {
		ret |= cam_ois_i2c_read(o_ctrl, 0x0103 + i, &RcvData,
			CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE);
		CAM_INFO(CAM_OIS, "RcvData[0x%x] %d", 0x0103 + i, RcvData);
		*(pBuf + i) = (RcvData & 0xFF);
	}
				...
				struct cam_ois_cal_addr *cal_addr)
{
	...
	CAM_INFO(CAM_OIS, "Write Reg : 0xE4, Data : 0x%x", cal_buf->rx_buf[0]);
	...
	CAM_INFO(CAM_OIS, "Write Reg : 0xE5, Data : 0x%x", cal_buf->af_position);
	...
	CAM_INFO(CAM_OIS, "Read Reg : 0x04, Data : %d", cal_buf->ideal_pCal[0]);
	CAM_INFO(CAM_OIS, "Read Reg : 0x06, Data : %d", cal_buf->ideal_nCal[0]);
	...
	CAM_INFO(CAM_OIS, "Read Reg : 0x04, Data : %d", cal_buf->ideal_pCal[1]);
	CAM_INFO(CAM_OIS, "Read Reg : 0x06, Data : %d", cal_buf->ideal_nCal[1]);
	...
	CAM_INFO(CAM_OIS, "ois read bypass1 af position %X", cal_buf->af_position);
	return 0;
}
				...
				struct cam_ois_cal_addr *cal_addr)
{
	...
	CAM_INFO(CAM_OIS, "Write Reg : 0xae, Data : 0x%x", cal_buf->tx_buf[0]);
	...
	CAM_INFO(CAM_OIS, "Read Reg : 0x04, Data : %d", cal_buf->current_pCal[0]);
	CAM_INFO(CAM_OIS, "Read Reg : 0x06, Data : %d", cal_buf->current_nCal[0]);
	...
	CAM_INFO(CAM_OIS, "Read Reg : 0x04, Data : %d", cal_buf->current_pCal[1]);
	CAM_INFO(CAM_OIS, "Read Reg : 0x06, Data : %d", cal_buf->current_nCal[1]);
	...
	uint16_t subdev_id, uint16_t *result)
{
	...
	CAM_INFO(CAM_OIS, "[#1] write for subdev %d", subdev_id);
	...
	CAM_INFO(CAM_OIS, "[#2] write for subdev %d", subdev_id);
...
int ps_notifier_cb(struct notifier_block *nb, unsigned long event, void *data)
{
	...
	CAM_INFO(CAM_OIS, "power supply callback");
